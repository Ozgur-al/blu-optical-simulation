...
autonomous: true
requirements: [ENS-01, ENS-02, ENS-03, ENS-04, ENS-05, ENS-06, ENS-07, ENS-08, ENS-09, ENS-10, ENS-11]
...
must_haves:
  truths:
    ...
    - "test_flux_tolerance_redrawn_per_member (ENS-11) is collected and xfail in Wave 0"
  artifacts:
...
Output: test_ensemble.py (11 tests, all FAIL or xfail), sim/ensemble.py (stub with correct API including build_mc_sample).
</objective>
...
No PySide6 imports. No GUI imports. This file is in `sim/` — must stay headless.
  ...
  <verify>
    <automated>python -c "from backlight_sim.sim.ensemble import apply_jitter, build_oat_sample, compute_oat_sensitivity, build_sobol_sample, build_mc_sample; print('stub imports OK')"</automated>
  </verify>
  <done>sim/ensemble.py exists with all 7 public functions defined (raising NotImplementedError), including build_mc_sample. Import succeeds without error.</done>
</task>
...
python -c "from backlight_sim.sim.ensemble import apply_jitter, build_oat_sample, compute_oat_sensitivity, build_sobol_sample, compute_sobol_sensitivity, build_mc_sample" exits 0.
</verification>

<success_criteria>
- sim/ensemble.py: module-level import succeeds; 7 public functions defined with NotImplementedError bodies (including build_mc_sample); NO PySide6/GUI imports
- test_ensemble.py: 11 test functions collected; all XFAIL in Wave 0; xfail markers use raises=(NotImplementedError, AttributeError, TypeError)
